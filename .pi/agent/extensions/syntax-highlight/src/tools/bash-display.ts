...
import { registerCleanup, registerTimer } from "../utils/disposable.ts";

const BASH_SPINNER_FRAMES = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"] as const;
const BASH_SPINNER_INTERVAL_MS = 200;
const BASH_SPINNER_STATE_KEY = "__syntaxHighlightBashSpinner";
const BASH_SPINNER_TOOL_CALL_ID_KEY = "__syntaxHighlightBashSpinnerToolCallId";
...
function highlightBashCommand(commandDisplay: string, theme: BashCallRenderTheme): string {
	try {
		const highlighted = highlightCode(commandDisplay, "bash").join("\n");
		return highlighted.length > 0 ? highlighted : theme.fg("accent", commandDisplay);
	} catch {
...
function buildBashCallText(
	...
	return `${spinnerPrefix}${theme.fg("toolTitle", theme.bold("$"))} ${highlightBashCommand(commandDisplay, theme)}${shellSuffix}${timeoutSuffix}${elapsedSuffix}`;
}
