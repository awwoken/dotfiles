...
import type { BtwAttemptResult, BtwErrorDetails } from "./types.ts";

interface RunBtwStreamOptions {
...
export function getErrorDetails(error: unknown): BtwErrorDetails {
...
export async function runBtwStream(options: RunBtwStreamOptions): Promise<BtwAttemptResult> {
	...
	try {
		...
		for await (const event of eventStream) {
			if (event.type === "text_start") {
				...
				return resultFromAssistantMessage(
					...
					{ message: event.error.errorMessage ?? (event.reason === "aborted" ? "Aborted" : "Model request failed") },
				);
			}
		}
		...
		return {
			...
			completedAt: new Date().toISOString(),
			answerText,
			error: { message: signal.aborted ? "Aborted" : "Model stream ended without a final message" },
		};
	} catch (error) {
		...
		return {
			...
			completedAt: new Date().toISOString(),
			answerText,
			error: signal.aborted && !details.message ? { message: "Aborted" } : details,
		};
	}
}
