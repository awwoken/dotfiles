...
export default function ollamaWebSearch(pi: ExtensionAPI) {
  pi.registerTool({
    name: "web_search",
    label: "Web Search",
    description: "Search the web for real-time information using local Ollama web_search, returning compact titles, URLs, and query-focused snippets.",
    promptSnippet: "Search the web and return compact titles, URLs, and query-focused snippets.",
    promptGuidelines: ["Use web_search for discovery; snippets are previews, not full source material."],
  ...
  pi.registerTool({
    name: "web_fetch",
    label: "Web Fetch",
    description: "Fetch a single URL using local Ollama web_fetch, returning the extracted page title, main content, and links.",
    promptSnippet: "Fetch a single URL and return extracted page content plus links.",
    promptGuidelines: [
    ...
    async execute(_toolCallId, params, signal) {
      ...
      try {
        ...
        return {
          content: [{ type: "text" as const, text: formatFetchResult(data, params.url, maxContentChars) }],
          details: {
            title: data.title,
            contentChars: data.content?.length ?? 0,
            returnedContentChars: Math.min(data.content?.length ?? 0, maxContentChars),
